...
@Component({
  selector: 'app-list-emergencies',
  templateUrl: './list-emergencies.component.html',
  styleUrls: ['./list-emergencies.component.scss'],
  providers: [
...
export class ListEmergenciesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.showEntries.controls['estadoEmergencia'].setValue("1")
  ...
  createEmergency() {
    this.router.navigate(['/emergencias/create-emergency']).then(() => { });
  }
      ...
      this.emergencyService.getEmergencies(d).subscribe((res: any) => {
        ...
        this.emergencies.map((x:any) =>{
          ...
          this.formularioService.getFormularios(d).subscribe((res: any) => {
            if (res.cantidad != 0) {           
              res.formularios.map((y:any)=>{
                if(y.tipoFormulario == 'EVALUACION-RAPIDA')
                  if(y.estado_formulario != 1) 
                    x.isEdit = false
              })              
            }
          });
  ...
  deleteEmergency(id: number, codigoSinpad: number) {
    this.alert.questionAlertConfirm('Est&aacute;s Seguro de Eliminar la Emergencia?', 'No volverás a visualizar está Emergencia', 'S&iacute;, Eliminar', TYPE_ALERT.QUESTION).then(
      (result) => {
        if (result.value) {
          this.emergencyService.deleteEmergency(id).subscribe((res: any) => {
  ...
  closeEmergency(id: number, codigoSinpad: number) {
